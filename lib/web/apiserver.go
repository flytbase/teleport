...
func (h *Handler) bindDefaultEndpoints() {
	...
	// OIDC connector handlers (FlytBase fork: OSS OIDC implementation)
...
// oidcLoginWeb handles the browser SSO login flow for OIDC connectors.
// FlytBase fork: OSS OIDC implementation mirroring the github equivalent.
...
// oidcLoginConsole handles the tsh/CLI SSO login flow for OIDC connectors.
// FlytBase fork: OSS OIDC implementation mirroring the github equivalent.
...
// oidcCallback handles the OAuth2/OIDC redirect back from the identity provider.
// FlytBase fork: OSS OIDC implementation mirroring the github equivalent.
func (h *Handler) oidcCallback(w http.ResponseWriter, r *http.Request, p httprouter.Params) string {
	...
	if err != nil {
		...
		// Try to recover the original client redirect URL so we can terminate
		// the failed SSO flow immediately.
		if requestID := r.URL.Query().Get("state"); requestID != "" {
	...
	// If a web session was created, set the cookie and redirect to the UI.
	if response.Req.CreateWebSession {
		logger.InfoContext(r.Context(), "Redirecting to web browser")
